/* 9/21 actual visits and separately labeled proposed later itinerary.
 * Display only: do not alter place identifiers, family reviews, or navigation.
 */
(function () {
  'use strict';
  function setup() {
    ...
    const h=document.createElement('h3');h.textContent='🗓️ 9/21 行程｜確認済みの訪問記録';box.append(h);
    const intro=document.createElement('div');intro.className='rr-note';intro.textContent='7:00 良粟商號 → 7:45 fruitos → 8:00 ㄇㄇ紫米飯糰 → ホテルへ戻る → 9:30 甜滿 → 天津蔥抓餅 → 永康街散策 → 12:00 雙月 → 14:00頃 双連現烤蛋糕 → 14:30頃 COOKIE886。以降は以前の予定で、訪問実績は未確認です。';box.append(intro);
    const actual=[
      ...
      {time:'14:00頃',name:'双連現烤蛋糕',note:'訪問済み。正式な店名・所在地はまだ未確認のため、別のお店と同一視せず記録。',area:'雙連｜現烤蛋糕（9/21 14:00頃訪問）',place:'双連現烤蛋糕'},
      {time:'14:30頃',name:'COOKIE886',note:'14:30頃に訪問済み。',area:'中山・赤峰街',place:'COOKIE886'}
    ];
    const later=[
      {time:'午後・時刻未確認',name:'中山・赤峰街のその他の散策',note:'以前の散策予定。COOKIE886の訪問は確認済みですが、その他の散策実績は未確認。',area:'中山・赤峰街',place:'赤峰街'},
    ...
    actual.forEach(addStep);
    const sub=document.createElement('div');sub.className='rr-subhead';sub.textContent='それ以降｜以前の予定（訪問実績は未確認）';box.append(sub);
    later.forEach(addStep);
    const extra=document.createElement('div');extra.className='rr-extra';extra.textContent='※ 下の「本日の行程表」画像は変更前です。実際の行程はこの記録と下の訪問済みカードが最新です。14:30以降のお店は訪問したと確認できるまで訪問済みとは扱いません。';box.append(extra);
