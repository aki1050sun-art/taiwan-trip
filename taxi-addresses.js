...
function setup(){
 ...
 if(hotel&&!hotel.querySelector('.hotel-address')){const line=document.createElement('span');line.className='hotel-address';line.textContent='住所：'+HOTEL;line.style.cssText='display:block;margin-top:4px;font-size:12px;line-height:1.5;font-weight:500;overflow-wrap:anywhere';hotel.appendChild(line)}
 document.querySelectorAll('.place').forEach(place=>{const name=getName(place),button=place.querySelector('.taxibtn');if(!name||!button||place.querySelector('.taxi-address-line'))return;const line=document.createElement('div');line.className='taxi-address-line';line.textContent='住所：'+addressText(name);line.style.cssText='font-size:12px;line-height:1.5;color:#475569;margin-top:5px;overflow-wrap:anywhere';const meta=place.querySelector('.meta');if(meta)meta.insertAdjacentElement('afterend',line);else place.querySelector('.name')?.insertAdjacentElement('afterend',line)});
